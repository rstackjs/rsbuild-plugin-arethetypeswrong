...
test('should throw when does resolve to types', async () => {
  ...
  expect(
    existsSync(
      path.join(import.meta.dirname, 'test-untyped-resolution-0.0.0.tgz'),
    ),
  ).toBeFalsy();
});
      ...
      plugins: [
        pluginAreTheTypesWrong({
          areTheTypesWrongOptions: {
            ignoreResolutions: ['bundler', 'node16-cjs', 'node16-esm'],
          },
        }),
      ],
    },
  ...
  const rsbuild = await createRsbuild({
    ...
    rsbuildConfig: {
      plugins: [
        pluginAreTheTypesWrong({
          areTheTypesWrongOptions: {
            ignoreResolutions: ['node16-cjs', 'node16-esm'],
          },
        }),
      ],
    },
...
test('should be able to ignore rule untyped-resolution', async () => {
  const rsbuild = await createRsbuild({
    ...
    rsbuildConfig: {
      plugins: [
        pluginAreTheTypesWrong({
          areTheTypesWrongOptions: {
            ignoreRules: ['untyped-resolution'],
          },
        }),
      ],
    },
  ...
  expect(
    existsSync(path.join(import.meta.dirname, 'test-no-resolution-0.0.0.tgz')),
  ).toBeFalsy();
});
